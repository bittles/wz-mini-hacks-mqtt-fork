...
${MOSQUITTO_SUB_BIN} -v -h "${MQTT_BROKER_HOST}" -p "${MQTT_BROKER_PORT}" -u "${MQTT_USERNAME}" -P "${MQTT_PASSWORD}" -t "${TOPIC_BASE}/#"  ${MOSQUITTOOPTS} | while read -r line ; do
  case $line in
      ...
      update_icamera_config "verSwitch" "1"
        ...
        "${TOPIC_BASE}/motion_detection/set ON")
      update_icamera_config "MASwitch" "1"
          if [ ! -z "$(grep "MASwitch=1" ${ICAMERA_CONFIG})" ]; then
                mqtt_publish "/motion_detection" "ON"
          fi
          commit_icamera_updates
        ;;

        "${TOPIC_BASE}/motion_detection/set OFF")
      update_icamera_config "MASwitch" "2"
          if [ ! -z "$(grep "MASwitch=2" ${ICAMERA_CONFIG})" ]; then
                mqtt_publish "/motion_detection" "OFF"
          fi
          commit_icamera_updates
        ;;
